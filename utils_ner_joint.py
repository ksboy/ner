...
class InputExample(object):
    ...
    def __init__(self, guid, words, token_type_ids, labels_i, labels_c):
            ...
            if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                if words:
                    if mode!='train':
                        ...
                        examples.append(InputExample(guid="{}-{}".format(mode, guid_index), words=words, token_type_ids=token_type_ids, labels_i=labels_i, labels_c = labels_c))
                    else:
                        ...
                        for _, start, end in entities:
                            ...
                            examples.append(InputExample(guid="{}-{}".format(mode, guid_index), words=words, token_type_ids=token_type_ids, labels_i=labels_i, labels_c = _labels_c))
                    guid_index += 1
                    words = []
                    labels_i, labels_c = [], []
            else:
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        label_ids_i = []
        label_ids_c = []
        for word, segment_id, label_i, label_c in zip(example.words, example.token_type_ids, example.labels_i, example.labels_c):
            word_tokens = tokenizer.tokenize(word)

            # bert-base-multilingual-cased sometimes output "nothing ([]) when calling tokenize with just a space.
            if len(word_tokens) > 0:
                tokens.extend(word_tokens)
                token_type_ids.extend([sequence_a_segment_id if not segment_id else trigger_token_segment_id]+ [sequence_a_segment_id] * (len(word_tokens) - 1))
